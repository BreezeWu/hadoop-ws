...
//  val rddFromHive = hiveContext.hql("SELECT cons_id, ym, volume_per_month FROM bigdata_arc_volume_perm_S98")
//  
...
//  // ############################## 不在这里再次描述了!!! ##############################
// 用于聚类的hive数据表是 BIGDATA_USER_INFO_S98_V_FOR_CLUSTERING
...
def tryKMeans(data: RDD[Vector], minK: Int = 2, maxK: Int, maxIterations: Int = 20) = {
	...
	// 任务初始时间
	val timeBegin00 = new java.util.Date()
	val timeBegin00Format = format.format(timeBegin00)
	println(s"----------------------- tryKMeans : begin @ ${timeBegin00Format} -----------------------")
	...
	for(k <- range) {
		...
		val timeBeginFormat = format.format(timeBegin)
		println(s"----------------------- k=${k} : begin @ ${timeBeginFormat} -----------------------")
		...
		// 任务时间记载
		val timeEndFormat = format.format(new java.util.Date())
		println(s"------------------------------ k=${k} : end @ ${timeEndFormat}------------------------------")

		timePairList = timePairList .:+(Tuple3(k.toString, timeBeginFormat, timeEndFormat))
	}

	// 任务结束时间
	val timeEnd00 = new java.util.Date()
	val timeEnd00Format = format.format(timeEnd00)
	println(s"----------------------- tryKMeans : end @ ${timeEnd00Format} -----------------------")

	timePairList = timePairList .:+(
			Tuple3(">>>", timeBegin00Format, timeEnd00Format)
		)
	
	println(s"[---------------------------------------------------------------------------------------------]")
	timePairList.reverse.foreach( x =>
	...
	val dateString = fileFormat.format(timeBegin00) // fileFormat.format(new java.util.Date())
