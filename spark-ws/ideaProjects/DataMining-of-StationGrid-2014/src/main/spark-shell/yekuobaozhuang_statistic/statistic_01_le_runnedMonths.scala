...
def ComputeUserCountListList_CHAOS(specialRecordRdd:RDD[(String, String, Float, scala.collection.immutable.IndexedSeq[(Int, Double)], String)],
  ...
  maxRunnedMonthsList.map(x => {
    ...
    val userCountList = percentList.map(y => {
      ...
      val curRdd = specialRecordRdd.filter(z => { // 运行时长过滤
        ...
        val filterFlag = if (maxMonthNo <= runnedMonths) true else false
