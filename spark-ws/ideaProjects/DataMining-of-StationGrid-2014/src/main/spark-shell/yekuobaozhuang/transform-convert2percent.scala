/**
 * 将 TableObject 进行转换.
 * 这里是将年中月用电量变换为： 月用电量平均值，
 * 1. 年内月度百分比 (本次分析未使用):
 *  (1) (年内)各月用电量/max(各月用电量)
 * 2. 按年度平均值化并将各年度平均值转换为百分比:
 *  (1) 各年月用电量平均值;
 *  (2) 各年月用电量平均值/max(各年月用电量平均值)
 */
...
case class TableObject_AverageMonthVolume_percent(index:Index,
...
// 求每年的月用电量平均值, 并将其转换为平均值的百分比
def convertTableObject2AveragePercent(item:TableObject):TableObject_AverageMonthVolume_percent = {
  ...
  */
  ...
  // ----------------------------------------------------------------------------
  return TableObject_AverageMonthVolume_percent(index, runnedMonths, runnedDays,
...
// 将月用点量转换为该用户最大值的百分必
// 数据集元素(percent)
case class TableObject_percent(index:Index, volumes:YearsVolumes, max:Double)

// 将 YearsVolumes 变换为百分比形式: 暂不使用
def convertTableObject2Percent(item:TableObject):TableObject_percent = {
  val index = item.index
  val volumes = item.volumes
  ...
  */
  ...
  return TableObject_percent(index, newVolumes, maxValue)
