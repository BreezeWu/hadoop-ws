...
implicit def Metric2ordered(x: Metric): Ordered[Metric] = new Ordered[Metric] {
  def compare(y: Metric): Int = (x.WSSSE).compare(y.WSSSE)
  /*
...
*/
// ---------------------------------------------------------------------------------------------------------------------------
// 聚类 Metric_SSRevaluator 信息
case class Metric_SSRevaluator(metric:Metric,
                               SSR: Double,  // 组间平方和, 或叫 簇间
                               evaluator:Double // 评价函数
                                ) {
  //def toRecord =  this.metric.toRecord+","+ SSR +","+ evaluator
  //def toRecord =  SSR +","+ evaluator+","+ this.metric.toRecord

  val dateFormat = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")    // "dd-MM-yyyy" "yyyy-MM-dd" "yyyy-MM-dd-HH-mm-ss"
  def toRecord = metric.k +","+ metric.maxIterations +","+ metric.WSSSE +
      ","+ SSR +","+ evaluator+
      ","+ dateFormat.format(metric.begin) +","+ dateFormat.format(metric.end) +","+ metric.model
}

implicit def Metric2ordered(x: Metric_SSRevaluator): Ordered[Metric_SSRevaluator] = new Ordered[Metric_SSRevaluator] {
  def compare(y: Metric_SSRevaluator): Int = (x.evaluator).compare(y.evaluator)
}

// 计算中心点/质心
import org.apache.spark.rdd.RDD
import org.apache.spark.mllib.linalg.Vector
def ComputeGravityCenter(data: RDD[Vector]):Vector = {
  import org.apache.spark.mllib.linalg.Vector
...
// 从 Metric 计算 Metric_SSRevaluator
def ConvertMetricList2SSRevaluator(data:RDD[Vector], metricList: List[Metric]):List[Metric_SSRevaluator] = {
  val nullResult:List[Metric_SSRevaluator] = null
  if (null == metricList) return nullResult

  val n = data.count
  val gravityVector = ComputeGravityCenter(data)

  def ComputeMetric_SSRevaluator(metric:Metric, n:Long, gravityVector:Vector):Metric_SSRevaluator = {
    val k = metric.k
    val WSSSE = metric.WSSSE
    val clusterCenters = metric.model.clusterCenters

    val SSR = ComputeSSR(gravityVector, clusterCenters)
    val evaluator = ComputeEvaluator(n, k, SSR, WSSSE)

    Metric_SSRevaluator(metric, SSR, evaluator)
  }

  metricList.map(x => ComputeMetric_SSRevaluator(x, n, gravityVector)).sortBy(x => x.evaluator)
}
// ---------------------------------------------------------------------------------------------------------------------------
...
def tryKMeansSmart(data: RDD[Vector], minK: Int = 2, maxK: Int, maxIterations: Int = 20):Account = {
  ...
  java.lang.ArrayIndexOutOfBoundsException: -1
    ...
    */
  val count = data.count
  if (minK > count) {    // 此时不能运行后续命令,否则回报错!
    DEBUGprint(s"minK:${minK} > count:${count}")
...
def writeAccount2LocalFile(account: Account, taskName:String = "feelingLucky", dir:String = null) = {
  ...
  // 文件路径
  val filenameMetric = taskName + "_kmeans_" + perfectK + "_metrics"
  val hdfsMetricInfoPath = rootpath + "/" + filenameMetric
  ...
  // 度量数据
  // metrics
  val hdfsPathsOfMetric = Tuple2(hdfsMetricInfoPath + "_executeOrder",
    hdfsMetricInfoPath + "_sortedByWSSSE")
  ...
  // 处理排序方式 两种都写入
  // 默认排序
  val distData1 = account.metricList.reverse.
    map(y => s"${y.k}, ${y.maxIterations}, ${y.WSSSE}, ${dateFormat.format(y.begin)}, ${dateFormat.format(y.end)}, ${y.model}")
  writeFunction(distData1, hdfsPathsOfMetric._1)

  // 按照WSSSE排序
  val distData2 = account.metricList.sortBy(i => i.WSSSE).
    map(y => s"${y.k}, ${y.maxIterations}, ${y.WSSSE}, ${dateFormat.format(y.begin)}, ${dateFormat.format(y.end)}, ${y.model}")
  writeFunction(distData2, hdfsPathsOfMetric._2)
  ...
  // 函数返回值
  //Tuple2(hdfsPathsOfMetric, hdfsClusterCentersInfoPath)
  Tuple2(hdfsPathsOfMetric._1, hdfsPathsOfMetric._2)
}

def writeSSRevaluator2LocalFile(list: List[Metric_SSRevaluator], taskName:String = "feelingLucky", dir:String = null) = {
  val rootpath = if (dir != null) dir else System.getenv().get("PWD")
  // 文件路径
  val filename = taskName + "_SSRevaluator"
  ...
  val diskData = list.map(x => x.toRecord)
  writeFunction(diskData, fullname)
