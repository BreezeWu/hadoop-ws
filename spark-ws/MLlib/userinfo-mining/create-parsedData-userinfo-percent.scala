// ----------------------------------------------------------------------------
// 从hive获取数据 ${datasetID}
//	用户信息和售电信息	BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE
// 依赖于 val datasetID="${datasetID}"

/*
-- 数据全集 = 有效数据 + 无效数据
-- 1. 有效数据
-- (1) 单月数据, BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_GOOD_M1 --> rddFromHive_GoodM1
-- (2) 双月数据, BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_GOOD_M2 --> rddFromHive_GoodM2

-- 2. 无效数据
-- (1) 前三个月都是 0或NULL， BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_ZERO_AND_NULL_F3 --> rddFromHive_BadF3
-- (2) 仅前两个月都是 0或NULL, BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_ZERO_AND_NULL_F2_EXCLUDE_F3  --> rddFromHive_BadF2ExcludeF3
*/
// ----------------------------------------------------------------------------
// ID
...
// 1. 有效数据
// 单月
val rddFromHive_GoodM1  = hiveContext.hql(s"SELECT ${vpmStrOf2013} FROM BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_GOOD_M1")
// 双月
val rddFromHive_GoodM2  = hiveContext.hql(s"SELECT ${vpmStrOf2013} FROM BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_GOOD_M2")

// 2. 无效数据
// 前3各月都是 0/NULL
val rddFromHive_BadF3  = hiveContext.hql(s"SELECT ${vpmStrOf2013} FROM BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_ZERO_AND_NULL_F3")
// 仅前两个月都是 0或NULL
val rddFromHive_BadF2ExcludeF3  = hiveContext.hql(s"SELECT ${vpmStrOf2013} FROM BIGDATA_USER_INFO_${datasetID}_ONEBIGTABLE_ZERO_AND_NULL_F2_EXCLUDE_F3")
...
// 应用于 rddFromHive_
val parsedData_GoodM1 = rddFromHive_GoodM1.map(x => sqlRow2Double_percent(x)).	// 去掉null,转换为Double
	map(x => Vectors.dense(x.toArray))		// 转变为Vector, 构建matrices

val parsedData_GoodM2 = rddFromHive_GoodM2.map(x => sqlRow2Double_percent(x)).	// 去掉null,转换为Double
	map(x => Vectors.dense(x.toArray))		// 转变为Vector, 构建matrices
	
val parsedData_BadF3 = rddFromHive_BadF3.map(x => sqlRow2Double_percent(x)).	// 去掉null,转换为Double
	map(x => Vectors.dense(x.toArray))		// 转变为Vector, 构建matrices

val parsedData_BadF2ExcludeF3 = rddFromHive_BadF2ExcludeF3.map(x => sqlRow2Double_percent(x)).	// 去掉null,转换为Double
	map(x => Vectors.dense(x.toArray))		// 转变为Vector, 构建matrices
