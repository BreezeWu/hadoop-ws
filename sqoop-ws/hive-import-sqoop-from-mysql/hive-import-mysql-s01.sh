...
#查看一下表列表
sqoop --options-file list-tables-mysql.sqoop --verbose
...
#sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_ARC_VOLUME_PERM_S01 --split-by 年月
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_ARC_VOLUME_PERM_S01 --split-by YM --boundary-query "
...
#	sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_RCVBL_FLOW_PM_S01 --split-by 年月
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_RCVBL_FLOW_PM_S01 --split-by YM
...
#sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_POWER_STEAL_PERY_S01 --split-by 年份

# 下面语句导出结果是42，但应该是85，原因在于原始数据里有年月是空的
#sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_POWER_STEAL_PERY_S01 --split-by Y

# 改为根据cons_id 85
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_POWER_STEAL_PERY_S01 --split-by cons_id
...
#sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_CHANGE_PAYTYPE_PY_S01 --split-by 年份
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_CHANGE_PAYTYPE_PY_S01 --split-by Y
...
# sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_TS_OR_PRCSCOPE_S01 --split-by 是否分时,是否阶梯
#
# TS_FLAG和LADDER_FLAG是汉字'否'和'是'，不能用来分区，新增了列ts_flag_i,ladder_flag_i。
# SQOOP 1.4.4 目前不能支持多列split。
# 	sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_TS_OR_PRCSCOPE_S01 --split-by ts_flag_i,ladder_flag_i
#sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_TS_OR_PRCSCOPE_S01 --split-by ts_flag_i  --boundary-query "
#    select  0000000026,8202921013 from dual
#"  --mapreduce-job-name BIGDATA_TS_OR_PRCSCOPE_S01

# 下面这个速度慢,但对于S01小数据集无所谓
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_TS_OR_PRCSCOPE_S01 --split-by cons_no

#分时电价电量   [ok]	hive中统计记录数 1130
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_VOLUME_OF_TS_S01 --split-by YM
#阶梯电价电量   [??]	hive中统计记录数 176628
sqoop --options-file hive-import-mysql.sqoop --table BIGDATA_VOLUME_OF_PRC_S01 --split-by YM

# ------------------------------------------------------------------------------
# 基础数据表只需导入一次

# 行业类型表 META_TRADE_TYPE	 [ok]	550
# sqoop --options-file hive-import-mysql.sqoop --table META_TRADE_TYPE --split-by TRADE_LEVEL

# 编码对照表 EPSA_LN.SA_PROP_LIST	[ok]	13549
# sqoop --options-file hive-import-mysql-forV3.sqoop --query 'SELECT * from EPSA_LN.SA_PROP_LIST WHERE $CONDITIONS' --hive-table META_PROP_LIST --split-by PROP_TYPE_ID --target-dir /user/hadoop/tmp-for-sqoop-hive/META_PROP_LIST
