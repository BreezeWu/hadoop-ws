...
// warning: previously defined class Recipe is not a companion to object Recipe.
...
def whatIs(a: Any): String = {
  val resultOpt: Option[(Any, Any)] = List.unapplySeq(a)
  if (resultOpt.isDefined) {
    val result = resultOpt.get
    val x = result._1
    val y = result._2
...
// We’ve been using == so far to determine value equality, but in the example above we use eq instead. This is testing for *reference equality*. You can do this on any object extending AnyRef. Incidentally, note that == doesn’t yet work the way we’d want it to for Recipe… we’ll fix that shortly.
...
// When you create a case class, sane implementations of all of the methods above are synthesized for you by the compiler. The fact that it’s so little effort to create, maintain and debug case classes should strongly encourage you to create rich domain models using them.
