...
// Wait, this looks suspiciously like switch-case from C. But notice that there’s no break statement needed, to prevent falling through to subsequent cases (sorry, no Duff’s device in Scala). If you want multiple cases to correspond to a single output, there’s a very convenient syntax:
...
monthName(13)       // 原定义的"=>"左边没有13
daysInMonth(13)     // 原定义的"=>"左边没有13
...
// This can be written differently with pattern guards (which sort of look like if-else expressions, but aren’t):
def whatIs(a: Any): String = a match {
  case n: Int if n % 2 == 0 =>
    "the even number " + n
  case n: Int =>
...
// Pattern matching is most compelling when used to extract features from various data structures. For example:
def whatIs(a: Any): String = a match {
  case (x, y, z) =>
...
whatIs(1L,2.2,3.3, 4)
whatIs(1L,2.2,3.3, "_.")
whatIs(List(1L,2.2,3.3, "_."))
whatIs(List(1L,2.2,3.3, "sfdfef"))
whatIs(List(1,2), List(10,20))
whatIs(List(1,2), List(10,20), 999)
whatIs(123.777)

whatIs(List((1,2), (10,20)))
...
def defensiveToInt(s: String): Int = try {
  ...
  case other:Throwable => other.printStackTrace(); 0
  case _: NumberFormatException => 0
}
...
def defensiveToInt(s: String): Int = try {
  ...
  case _: NumberFormatException => 0
    ...
    //return "finally"      // 与Int不匹配
    //return Int.MaxValue   // 导致所有都返回 Int.MaxValue
    println("finally")
}
...
// In this example, hello and world are actually both variable patterns, and in fact the left hand side of the <-s in for-expressions are always some sort of pattern. 

// This example shows an extractor pattern matching a Tuple2, with two variable patterns inside:
...
// Pattern matching is one of the most powerful constructs provided by Scala, so it’s often good engineering practice to model your problem domain by decomposition into cases. This becomes especially compelling when paired with case classes, which you’ll see in the next lesson.
