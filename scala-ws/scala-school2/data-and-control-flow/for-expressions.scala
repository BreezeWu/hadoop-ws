...
// (1) Explicitly indexing into the list is prone to bugs; you’re likely to have an off-by-one error or accidentally use the wrong index variable in the wrong collection.
// (2) The time complexity of this algorithm is O(n2), because indexing into a List is a linear-time operation, not constant-time (note: if we used a more appropriate collection type, such as Vector, this wouldn’t be an issue).
// (3) It’s not nearly DRY enough: noisyWords appears two times and quietWords appears three times.
...
// Aha, immutable and DRY. The for-yield expression indicates transformation: it directly produces a new collection, where each element is transformed from a corresponding element in the original collection. The behavior varies depending on the type of the original collection, but in this case where you start with a List[String], the for-yield expression produces a List[String] in the order you’d expect. In many other programming languages, this is referred to as “list comprehensions.”
...
// One nice thing about for-loops and for-expressions is that they can contain multiple generators, producing a similar effect to nested loops:
...
// You can also directly assign bindings (equivalent to vals) inside the for { ... }, not directly generated from elements in a collection:
val salutations = for {
  hello <- List("hello", "greetings")
  world <- List("world", "interwebs")
...
// … which isn’t necessarily useful by itself, but becomes very handy when you need to filter some of the results:
val salutations = for {
  hello <- List("hello", "greetings")
  world <- List("world", "interwebs")
